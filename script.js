...
// Critical deduplication function placed before initialization
...
// Automatically clear out duplicates right here
...
let currentFilterHours = 999999; // FIX: Changed default from 24 to ALL TIME
            ...
            if (result.status === 'fulfilled' && result.value.html) {
                ...
                doc.querySelectorAll('.tgme_widget_message').forEach(msg => {
                    ...
                    if(textEl && dateEl) {
                        ...
                        // FIX: Re-added the logic to scrape images and videos for live news
                        let mediaHTML = '';
                        const photoWrap = msg.querySelector('.tgme_widget_message_photo_wrap');
                        if (photoWrap && photoWrap.style.backgroundImage) {
                            const urlMatch = photoWrap.style.backgroundImage.match(/url\(['"]?(.*?)['"]?\)/);
                        ...
                        const newNewsObj = { 
                            ...
                            mediaHTML: mediaHTML, // FIX: Actually assigning the media to the object now
...
window.renderNewsFeeds = function() {
    ...
    const renderList = (posts, elId, limit, isSummary = false) => {
        ...
        posts.slice(0, limit).forEach(p => {
            ...
            if(isSummary) {
                ...
                // FIX: Added the ${p.mediaHTML} logic below so it actually prints the media into the card
                html += `
    ...
    renderList(processedNews.filter(p => p.text.includes('Iran') || p.text.includes('IRAN') || p.channel.includes('IRAN')), 'iran-news-feed', 20, false);
...
function renderMapData() {
    ...
    const filtered = globalIntelData.map(d => { 
        ...
        .sort((a,b) => a.timeAgo - b.timeAgo).slice(0, 500); // FIX: Increased slice limit to ensure all dots show
    ...
    filtered.forEach(strike => {
        ...
        // FIX: Explicitly sizing the MapLibre marker so it never collapses
        elContainer.style.width = '20px';
        elContainer.style.height = '20px';
...
window.onload = () => {
    ...
    // FIX: Set the filter state to ALL TIME right as the page loads
    setFilter(999999); 
